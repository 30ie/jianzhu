	...
	$.ajax({
		...
		success: function(msg){
			...
			$(".u_inst").click(function(){
				...
				$('#frmWindow').remove();
				$('body').append('<div id="frmWindow"><div class="topBar">升级服务验证<div class="pwClose"></div></div><div class="border"><div class="ntc">请输入授权用户帐号和密码，即您在PHPWEB软件官方网站的登录帐号和密码：</div><div class="ntc">授权用户：<input id="user" type="text" class="input" value="'+$('#phpwebUser')[0].value+'" /><br />授权密码：<input id="passwd" type="password" class="input" /><br /><input id="updatebutton" type="button" class="button" value="提交"></div></div></div>');
