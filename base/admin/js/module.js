...
$(document).ready(function(){
				...
				success: function(msg){
					if(msg.substr(1,6)=="option"){
						...
						$("#instbutton").click(function(){
							...
							$('#frmWindow').remove();
							$('body').append('<div id="frmWindow"><div class="topBar">软件授权验证<div class="pwClose"></div></div><div class="border"><div class="ntc">请输入授权用户帐号和密码，即您在PHPWEB软件官方网站的登录帐号和密码：</div><div class="ntc">授权用户：<input id="user" type="text" class="input" value="'+$('#phpwebUser')[0].value+'" /><br />授权密码：<input id="passwd" type="password" class="input" /><br /><input id="authcoltype" type="hidden" value="'+icoltype+'" /><br /><input id="authbutton" type="button" class="button" value="提交"></div></div></div>');
   ...
   $(".uninstall").click(function(){
		var qus=confirm("模块卸载不可恢复！卸载时将删除该模块的全部数据库记录，模块的文件系统将被重命名保留，确定要删除模块吗?")
		if(qus!=0){
